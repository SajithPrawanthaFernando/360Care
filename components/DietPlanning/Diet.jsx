...
const mealDatabase = {
    breakfast: [
        ...
        { name: 'Eggs and Spinach', image: Eggs, minBudget: 1800, maxBudget: 7000, bmiRange: [4.0, 5.0] ,calories: 250 ,budget:700},
      ],
      lunch: [
    ...
    const Diet = () => {
      ...
      const [dietPlan, setDietPlan] = useState({ breakfast: null, lunch: null, dinner: null });
    ...
    const generateDietPlan = (bmi, budget) => {
        ...
        setDietPlan({
            breakfast: breakfastOptions.length > 0 ? breakfastOptions.slice(0, 2) : null,
  ...
  return date.toLocaleDateString('en-US', options);
    ...
    return (
        <ScrollView style={styles.container}>
            ...
            <ImageBackground source={m} style={styles.imageBackground} imageStyle={styles.imageBackgroundCurve}>
                ...
                <View style={styles.mealRow}>
                    {dietPlan.breakfast ? (
                        dietPlan.breakfast.map((meal, index) => (
                            <TouchableOpacity key={index} onPress={() => setSelectedBreakfast(meal)}>
                                <ImageBackground source={meal.image} style={styles.mealImage} imageStyle={styles.mealImageCurve}>
                                    <Text style={styles.mealName}>{meal.name}</Text>
                                </ImageBackground>
                            </TouchableOpacity>
                        ))
                    ) : (
                        <Text>No suitable brennnakfast found</Text>
                    )}
                </View>
            </View>
                ...
                <View style={styles.mealRow}>
                    {dietPlan.lunch ? (
                        dietPlan.lunch.map((meal, index) => (
                            <TouchableOpacity key={index} onPress={() => setSelectedLunch(meal)}>
                                <ImageBackground source={meal.image} style={styles.mealImage} imageStyle={styles.mealImageCurve}>
                                    <Text style={styles.mealName}>{meal.name}</Text>
                                </ImageBackground>
                            </TouchableOpacity>
                ...
                <View style={styles.mealRow}>
                    {dietPlan.dinner ? (
                        dietPlan.dinner.map((meal, index) => (
                            <TouchableOpacity key={index} onPress={() => setSelectedDinner(meal)}>
                                <ImageBackground source={meal.image} style={styles.mealImage} imageStyle={styles.mealImageCurve}>
                                    <Text style={styles.mealName}>{meal.name}</Text>
                                </ImageBackground>
                            </TouchableOpacity>
